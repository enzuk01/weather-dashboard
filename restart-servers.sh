...
restart_direct() {
  ...
  # Always start from the script directory
  ...
  case "$1" in
    "backend")
      print_message "Restarting backend service..."
      # Terminate any existing process more reliably
      ...
      if lsof -ti:$BACKEND_PORT > /dev/null; then
        ...
        print_warning "Backend may not have started correctly. Check logs/backend.log"
      fi
      ;;
    ...
    "frontend")
      ...
      # Start frontend with absolute paths
      ...
      # Wait briefly and verify it's starting
      sleep 3
      if ps -p $FRONTEND_PID > /dev/null; then
        print_success "Frontend process is running"
      else
        print_warning "Frontend may not have started correctly. Check logs/frontend.log"
      fi
      ;;
    ...
    *)
      ...
      # Wait briefly for backend to start before starting frontend
      sleep 2
      if lsof -ti:$BACKEND_PORT > /dev/null; then
        print_success "Backend is running on port $BACKEND_PORT with PID $BACKEND_PID"
      else
        print_warning "Backend may not have started correctly. Check logs/backend.log"
      fi
